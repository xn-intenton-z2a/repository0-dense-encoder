...
6) tests/unit/coverage.assertion.test.js (optional)
- Purpose: provide a fast failing assertion if coverage drops below project thresholds. This helper may be used by CI.
- Behaviour:
  - Run the coverage reporter output generated by npm run test:unit and assert lines >= 50% and branches >= 30%.
...
- [ ] The test suite, when executed with npm run test:unit, produces coverage results with line coverage >= 50% and branch coverage >= 30.
- [ ] npm test runs the test suite and tests are written to be deterministic; CI should enforce these checks.
