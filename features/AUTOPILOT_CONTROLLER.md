...
- [ ] The autopilot lands safely (landed true and crashed false) with the default initial state (altitude=1000, velocity=40, fuel=25). Tests: tests/unit/autopilot.test.js::default-case must assert landed true and final velocity ≤ 4.
- [ ] The autopilot produces safe landings for at least ten distinct initial condition triples sampled across altitude 500–2000m, velocity 20–80 m/s, fuel 10–50 units, where those triples are physically possible to save. Tests: tests/unit/autopilot.test.js must include at least ten representative triples and assert landed true for saveable cases.
