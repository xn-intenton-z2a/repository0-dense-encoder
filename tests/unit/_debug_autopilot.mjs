import('./../../src/lib/main.js').then(({ createState, simulate, autopilot }) => {
  const combos = [
  ...
  for (const cfg of combos) {
    ...
    console.log(JSON.stringify({ cfg, last: { landed: last.landed, crashed: last.crashed, velocity: last.velocity, fuel: last.fuel, altitude: last.altitude }, traceLen: trace.length }));
  }
}).catch((e) => {
  console.error(e);
  process.exit(1);
